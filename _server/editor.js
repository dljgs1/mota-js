function editor() {
    ...
    this.dom={
        ...
        bg2c : document.getElementById('bg2'),
        bgCtx : document.getElementById('bg').getContext('2d'),
        bg2Ctx : document.getElementById('bg2').getContext('2d'),
        fgc : document.getElementById('fg'),
        fg2c : document.getElementById('fg2'),
        fgCtx : document.getElementById('fg').getContext('2d'),
        fg2Ctx : document.getElementById('fg2').getContext('2d'),
        ...
        layerMod4:document.getElementById('layerMod4'),
        layerMod5:document.getElementById('layerMod5'),
...
editor.prototype.init = function (callback) {
    ...
    main.init('editor', function () {
        ...
        core.dymCanvas.eui = editor.dom.eui.getContext('2d');
        afterMainInit();
    });
    ...
    var afterMainInit = function () {
        ...
        editor_file = editor_file(editor, function () {
            ...
            core.changeFloor(lastFloorId, null, core.firstData.hero.loc, null, function () {
                ...
                var textureData = [];
                var img = 'enemys';
                for(var i in core.material.icons[img]){
                    let tmp = main.core.material.enemys[i].texture;
                    if(!tmp)continue;
                    textureData.push(tmp);
                }
                textureData = textureData.map((n)=>{return 'textures/'+n});
                core.loadImages(textureData, editor.textureData,
                    ()=>{
                        afterCoreReset();
                    })
            }, true);
...
editor.prototype.updateMap = function () {
    ...
    core.status.maps[core.status.floorId].blocks = blocks;

    var updateMap = function () {
        SceneManager.mapScene.drawMap(core.status.floorId);
        return;
    ...
    */
    ...
    for (var ii = 0; ii < imgNames.length; ii++) {
        ...
        if(img =='enemys'){
            textureData = {};
            for(var i in core.material.icons[img]){
                if(main.core.material.enemys[i].texture){
                    textureData[main.core.material.icons[img][i]] =
                        editor.textureData['textures/'+main.core.material.enemys[i].texture];
                }
            }
        }
        if (img == 'terrains') {
        ...
        if (editor.uivalues.folded) {
            ...
            for (var i = 0; i < subimgs.length; i+=frames) {
                var dd = false;
                if(img == 'enemys'){
                    let w = 32, h = editor.uivalues.foldPerCol * per_height;
                    let ctx = core.createCanvas('tmp', 0, 0, w, h, -1);
                    ctx.drawImage(images[img],
                        0, ~~(i/frames) * editor.uivalues.foldPerCol * per_height,
                        w, h,
                        0,0,w,h
                        );
                    var drawTextureImage = function (idx, texture, x, y, page) {
                        idx -= page * editor.uivalues.foldPerCol;
                        ctx.clearRect(0, idx * 32, 32, 32);
                        ctx.drawImage(
                            texture, 0, 0, texture.width/4, texture.height/4,
                            0, idx*per_height, 32, 32
                        );
                    };
                    var drawTextureAll = function (image,x,y) {
                        image.src = ctx.canvas.toDataURL("image/png");
                        image.style.left = x + 'px';
                        image.style.top = y + 'px';
                        iconImages.appendChild(image);
                    };
                    for(let k in textureData){
                        k = ~~k;
                        let st = ~~((i + frames) * editor.uivalues.foldPerCol / frames);
                        if(k<st && k>=(st - editor.uivalues.foldPerCol)){
                            drawTextureImage(k,
                                textureData[k],
                                nowx, 0, ~~(i/frames));
                            dd = true;
                    ...
                    if(dd)drawTextureAll(subimgs[i], nowx, 0);
                }

                if(!dd)drawImage(subimgs[i], nowx, 0);// 原始图
                nowx += 32;
            }
        }
        else {
