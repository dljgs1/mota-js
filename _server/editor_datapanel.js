editor_datapanel_wrapper = function (editor) {
    ...
    editor.uifunctions.copyPasteEnemyItem_func = function () {
        ...
        var addNewSpecial = document.getElementById('addNewSpecial');
        var removeSpecial = document.getElementById('removeSpecial');
        addNewSpecial.onclick = function() {
            var cls = (editor_mode.info || {}).images;
            if (editor_mode.mode != 'enemyitem' || (cls != 'enemys' && cls != 'enemy48' && cls != 'items')) return;
            var id = editor_mode.info.id;
            var s = core.enemys.enemys[id].special;
            if(s){
                if(s instanceof Array){
                    s.push({'id':0});
                }else {
                    core.enemys.enemys[id].special = [{'id':0}];
                }
            }else {
                core.enemys.enemys[id].special = [{'id':0}];
            }
            editor.file.saveSetting('enemys', [], function(e){
                editor_mode.onmode('nextChange');
                editor_mode.onmode('enemyitem');
            });
        }
        removeSpecial.onclick = function() {
            var cls = (editor_mode.info || {}).images;
            if (editor_mode.mode != 'enemyitem' || (cls != 'enemys' && cls != 'enemy48' && cls != 'items')) return;
            var id = editor_mode.info.id;
            var s = core.enemys.enemys[id].special;
            if(s){
                if(s instanceof Array){
                    ...
                    core.enemys.enemys[id].special = null;
                }
            }
            editor.file.saveSetting('enemys', [], function(e){
                editor_mode.onmode('nextChange');
