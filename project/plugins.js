var plugins_bb40132b_638b_4a9f_b028_d3fe47acc8d1 = 
{
	...
	this._afterLoadResources = function () {
		...
		// }
		// core.extensions._loadJs('project/specials.js');
		// core.extensions._loadJs('plugins/componentUI.js');
		// core.extensions._loadJs('plugins/skillTree.js');
		// core.extensions._loadJs('plugins/monsters.js');
...
"blocklyUp":function(){//事件块升级
	// 1 事件转向
	
	events.prototype._action_turnEvent = function (data, x, y, prefix) {
		var direction = data.direction;
		var floorId = data.floorId || core.status.floorId;
		if(!data.loc || data.loc.length == 0){ //不填 全部转向
			core.status.maps[floorId].blocks.forEach(function(b){
				if(b.event.cls != 'npc48')return;
				if(!b.disable && core.maps.blocksInfo[b.id].faceIds){
					var id = core.maps.blocksInfo[b.id].faceIds[direction];
					core.setBlock(id, b.x, b.y);
				}
			});
		}else
		for(var ii in data.loc){
			var blk = core.getBlock(data.loc[ii][0],data.loc[ii][1],floorId);
			if(!blk)return core.doAction();
			var id = core.maps.blocksInfo[blk.block.id].faceIds[direction];
			core.setBlock(id, data.loc[ii][0], data.loc[ii][1],floorId);
		}
		core.doAction();
	}
	maps.prototype._moveBlock_updateDirection = function (blockInfo, moveInfo) {
		...
		if (currid && !core.hasFlag('_fix_direction')) {
