    ...
    value: function makePointer() {
      ...
      var pointer = {
        ...
        moveHandler: function moveHandler(event) {
          ...
          if(isVertical){
            this._x = event.pageY - element.offsetTop;
            this._y = element.clientWidth - (event.pageX - element.offsetLeft);
          }else{
          ...
          if(this.isDown){
              if(!this.isDrag){
                this.isDrag = true;//Date.now() - this.downTime> 100 &&
              }
          }else this.isDrag = false;
          if(this.isDrag && this.drag)this.drag();
        ...
        touchmoveHandler: function touchmoveHandler(event) {
          ...
          if(isVertical){
            this._x = event.targetTouches[0].pageY - element.offsetTop;
            this._y = element.clientWidth - (event.targetTouches[0].pageX - element.offsetLeft);
          }else{
        ...
        hitTestSprite: function hitTestSprite(sprite) {
          ...
          if (!sprite.circular) {
            ...
            //coordinates
              if(isVertical){
                  xAnchorOffset += this.element.clientWidth;
              }
